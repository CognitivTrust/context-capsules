...
```bash
# Initialize empty capsule storage under .capsule/
capsule init
...
| --- | --- |
| `capsule init` | Create empty capsule storage under `.capsule/`. |
...
- **Local-first.** Zero network I/O: no telemetry, no phone-home, no API calls. The CLI and core engine stay on-device.
